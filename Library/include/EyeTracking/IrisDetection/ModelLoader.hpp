    ...
    class ModelLoader {
        public:
            ...
            */
            ...
            // Rule of 5: Move constructors were missing
            ModelLoader(ModelLoader&&) noexcept = default;
            auto operator=(ModelLoader&&) noexcept -> ModelLoader& = default;


            // =========================================================================
            // 1. Convenience Overloads (Non-Virtual)
            //    Callers can omit arguments cleanly. These do NOT trigger clang-tidy.
            // =========================================================================
            ...
            // below are the virtual functions - derived class override these without any default args
            ...
            */
            [[nodiscard]]
            auto getInputShape(int index = 0) const -> std::vector<int>;
            ...
            */
            [[nodiscard]]
            auto getInputData(int index = 0) const -> float*;
            ...
            */
            [[nodiscard]]
            auto getInputSize(int index = 0) const -> size_t;

            /*
            Get number of inputs needed to run inference. 
            */
            [[nodiscard]]
            auto getNumberOfInputs() const -> int;
            ...
            */    
            [[nodiscard]] 
            auto getOutputShape(int index = 0) const -> std::vector<int>;
            ...
            */
            [[nodiscard]]
            auto getOutputData(int index = 0) const -> float*;
            ...
            */
            [[nodiscard]]
            auto getOutputSize(int index = 0) const -> size_t;

            /*
            Get number of outputs from inference.
            */
            [[nodiscard]]
            auto getNumberOfOutputs() const -> int;
            ...
            */
            [[nodiscard]]
            virtual auto loadOutput(int index) const -> std::vector<float>;
        ...
        private:
            ...
            */
            [[nodiscard]]
            auto isIndexValid(int index, char c = 'i') const -> bool;

            /*
            Check if all inputs have been loaded
            */
            [[nodiscard]]
            auto isAllInputsLoaded() const -> bool;
            ...
            */
            [[nodiscard]]
            auto preprocessImage(const cv::Mat& in, int idx) const -> cv::Mat;

            /*
            Convert image of type CV_8UC3 or CV_8UC4 to RGB format
            */
            [[nodiscard]]
            auto convertToRGB(const cv::Mat& in) const -> cv::Mat;
